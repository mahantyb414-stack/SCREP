...
SCROLL_ITERS = 20          # max scroll iterations in feed (reduced for speed)
SCROLL_SLEEP = (0.15, 0.3) # (min, max) sleep between scroll steps (reduced)
JITTER_PRE   = (0.2, 0.5)  # pre-navigate jitter per worker (reduced)
JITTER_GOTO  = (0.1, 0.3)  # after page load settle (reduced)
...
class GoogleMapsKeywordScraper:
    ...
    MAX_RESULTS = 30  # Reduced for free tier testing
    ...
    async def scrape_all_async(
        ...
        async with pool.acquire() as (_, collect_page):
            ...
            if not business_urls:
                ...
                kw_parts = self.search_keyword.replace("Photographers in ", "", 1).split()
                # try progressively shorter suffixes
                for trim in range(1, len(kw_parts)):
                    fallback_kw = "Photographers in " + " ".join(kw_parts[trim:])
...
#     SEARCH_KEYWORD = "Pre wedding Photographersin Titagarh Kolkata West Bengal"
#     OUTPUT_JSON    = os.path.join(
#         os.path.dirname(os.path.abspath(__file__)), "Data", "Titagarh_Photographers.json"
#     )
#     scraper = GoogleMapsKeywordScraper(SEARCH_KEYWORD, OUTPUT_JSON)
#     scraper.scrape_all(max_workers=16)